...
Для воспроизводимости «текущая дата» явно задаётся в тесте. Месяц определяется интервалом от начала месяца включительно до начала следующего исключительно. Питомец с несколькими записями к одному врачу выводится один раз по его идентификатору; одинаковые клички разных питомцев сохраняются.
